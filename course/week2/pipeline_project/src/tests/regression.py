...
def build_regression_test(system, loader):
  ...
  for batch in tqdm(loader):
    ...
    # NB: rather than cast from array to list and extend the lists,
    # its more efficient to just build a list of numpy arrays and then
    # concatenate them at the end
    batch_is_correct = preds == labels
    batch_loss = F.cross_entropy(logits, labels, reduction='none').numpy()
    losses.append(batch_loss)
    is_correct.append(batch_is_correct)

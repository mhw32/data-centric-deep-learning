...
def get_hist_score(tr_probs, te_probs, bins=10):
  ...
  bin_diffs = bin_edges[1:] - bin_edges[:-1]
  np.testing.assert_almost_equal(tr_heights.dot(bin_diffs), 1)
  np.testing.assert_almost_equal(te_heights.dot(bin_diffs), 1)
  intersect_height = np.minimum(tr_heights, te_heights)
  bin_diffs = bin_edges[1:] - bin_edges[:-1]
  return np.dot(intersect_height, bin_diffs)
...
def get_vocab_outlier(tr_vocab, te_vocab):
  ...
  # score: float (between 0 and 1)
  # ============================
  tr_keys = set(tr_vocab.keys())
  te_keys = set(te_vocab.keys())
  print(len(tr_vocab), len(te_vocab))
  return len(te_keys.difference(tr_keys)) / len(te_keys)
...
class MonitoringSystem:
  ...
  def calibrate(self, tr_probs, tr_labels, te_probs):
    ...
    ir.fit(tr_probs.numpy(), tr_labels.numpy())
    tr_probs_cal = ir.predict(tr_probs.numpy())
    te_probs_cal = ir.predict(te_probs.numpy())
    return torch.Tensor(tr_probs_cal), torch.Tensor(te_probs_cal)
